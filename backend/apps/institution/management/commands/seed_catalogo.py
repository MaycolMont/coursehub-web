...
from apps.accounts.models import Rango
...
CDIA_MATERIAS = [
    'INDG1033', 'CCPG1043', 'IDIG1006',
    'TICG1018', 'CDIAG1003', 'CCPG1052', 'IDIG1007',
...
class Command(BaseCommand):
    help = 'Puebla la BD con el catálogo académico de ESPOL (sin recursos demo)'
    ...
    def handle(self, *args, **options):
        ...
        self._seed_rangos()
        self.stdout.write(self.style.SUCCESS('Catálogo académico completado.'))

    def _seed_facultades(self):
        for pk, nombre in FACULTADES:
            Facultad.objects.update_or_create(id=pk, defaults={'nombre': nombre})
    ...
    def _seed_carreras(self):
        for pk, nombre, facultad_id in CARRERAS:
            Carrera.objects.update_or_create(
                id=pk, defaults={'nombre': nombre, 'facultad_id': facultad_id},
            )
    ...
    def _seed_materias(self):
        ...
        for pk, codigo, nombre in MATERIAS:
            ...
            self.codigo_map[codigo] = materia
        self.stdout.write(f'  {len(MATERIAS)} materias.')

    def _seed_carreras_materias(self):
        count = 0
        for codigo in CDIA_MATERIAS:
            materia = self.codigo_map.get(codigo)
            if not materia:
                continue
            _, created = CarreraMateria.objects.get_or_create(carrera_id=1, materia=materia)
            count += created
        for codigo in COMP_MATERIAS:
            materia = self.codigo_map.get(codigo)
            if not materia:
                continue
            _, created = CarreraMateria.objects.get_or_create(carrera_id=2, materia=materia)
            count += created
        self.stdout.write(f'  {count} relaciones carrera-materia.')

    def _seed_profesores(self):
        for pk, nombre in enumerate(PROFESORES, start=1):
            Profesor.objects.update_or_create(id=pk, defaults={'nombre': nombre})
        self.stdout.write(f'  {len(PROFESORES)} profesores.')

    def _seed_rangos(self):
        for nombre, karma_minimo in RANGOS:
            Rango.objects.update_or_create(
                nombre_rango=nombre, defaults={'karma_minimo': karma_minimo},
            )
        self.stdout.write(f'  {len(RANGOS)} rangos.')